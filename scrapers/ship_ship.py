...
class ShipShipScraper(BaseScraper):
    site_name = "ship_ship"
    base_url = "https://irkutsk.ship-ship.ru"
    catalog_url = "https://irkutsk.ship-ship.ru/tyres/"
    max_pages = 20
    ...
    async def parse_products(self, html: str) -> list[ProductDTO]:
        ...
        for card in soup.select(".product-card"):
            ...
            price_el = card.select_one(".product-card__price-value")
            ...
            if price is None:
                continue
            old_el = card.select_one(".product-card__price-old")
...
if __name__ == "__main__":
    result = asyncio.run(ShipShipScraper().run())
    print(f"Parsed {len(result)} products from {ShipShipScraper.site_name}")
